...
def encode(plaintext, shift_num):
	...
	for i in plaintext:
		...
		if (plaintext[i]).isaplha(): #if it is a letter of the alphabet
			ord(plaintext[i]) #obtain the number it corresponds to,
			return(plaintext[i])
			plaintext[i] = x
			x = x + shift_num #add the shift number to it

			if (97 <= x >= 122):
				chr(x) #convert it to a letter
				print (x) #print the new letter

		else: #if not a letter of the alphabet, return it
			print(plaintext[i])
